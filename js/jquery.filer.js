 ...
 */
 //GLOBAL VARIABLES 
 var clickOnMainAreaFlag=false; 
 var numberOfFilesToUpload=4;
 var permitedFiletypes=["xml", "nessus"];
        ...
        return this.each(function(t, r) {
            ...
            f = {
                ...
                _clickHandler: function() {
                    ...
                    if(clickOnMainAreaFlag==false)
                    {
                        s.click();

                    }
                    clickOnMainAreaFlag=false; 
            ...
            _filesCheck: function() {
                var s = 0;
             /*  If number of files is bigger than maximum allowed number upload is not permited
                f._itFl.length - number of files that is wanded to upload with multiple upload option
                f.files.length- nr of total files*/
                console.log(f._itFl.length+f.files.length);
                if (f._itFl.length+f.files.length >numberOfFilesToUpload ) {
                ...
                for (var t = 0; t < f.files.length; t++) {
                    ...
                    //if file is of an allowed type (specified in permitedFiletypes Array)
                    if (permitedFiletypes.indexOf(x) <= -1) {

                      file._pendRemove = true;
                      n.dialogs.alert(f._assets.textParse(n.captions.errors.filesType));
                      return false;
                  } 
            ...
            for (var t = 0; t < f.files.length; t++) {
                ...
                if (uploaded_files_map.indexOf(f.files[t].name) > -1) {
                    file._pendRemove = true;
                   n.dialogs.alert(f._assets.textParse(n.captions.errors.filesExist));
                    console.log(uploaded_files_map);

                    return false;
                } else {
                    uploaded_files_map.push(f.files[t].name);
                   console.log("fies successfully uploaded the map is: "+uploaded_files_map);
                }
